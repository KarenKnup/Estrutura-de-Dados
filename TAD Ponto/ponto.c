...
//é aqui que se desenvolve os protótipos que o usuário vai usar em "ponto.h"
//os tipos ".h" e ".c" DEVEM TER  o mesmo nome
...
char* pto_getPonto(Ponto* p){
  char * ptAux;
  int qtCaracteres= sizeof(*p) + 8; //8 = 2 "(" + 5 " " + 1 "\0"
  ptAux=(char*)malloc(qtCaracteres);
  sprintf(ptAux,"(%.2f , %.2f)",p->x,p->y);//string scanf
  return ptAux;
