...
tLSECC* cria_sublista (tLSECC* l1, int qtde) {
    if (l1->cabeca == NULL || qtde <= 0 || l1->qtdNos<qtde) {
        return NULL;
    }

    tLSECC* l2 =cria_lista(l1->isClassif, l1->comRepet);
    tNo* atual = l1->cabeca->prox, *no;
    int count = 1;
  
    while (count != l1->qtdNos) {  
      if(atual->inf == qtde){
          //sem incluir o número digitado:          
          while (count != l1->qtdNos && atual!= l1->cabeca){
             atual = atual->prox;
            count++;
            inserir(l2,atual->inf);
          }
        
        //incluindo o número digitado:
       /* while (count != l1->qtdNos+1 && atual!=l1->cabeca){
          inserir(l2,atual->inf);
          atual = atual->prox;
            count++;
          }*/             
